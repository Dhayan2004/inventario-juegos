...
> *"Validá la idea antes de construir la solución perfecta."*
...
- **Cuándo usar:** cuando necesitás mostrar algo funcionando rápido, validar con usuarios reales, o decidir si vale la pena construir el SaaS completo.
...
**Skipped:** UX Research (#4), UX Design (#6), UI Design Workflow (#7), Security Audit (#9). Para producción real → migrar a `routes/saas-completo.md` post-validación.
...
- **Tiempo:** ~20 min
- **Qué hace:** evaluación rápida de viabilidad técnica (Forja Golden Path fit), negocio y marketing.
...
- **Tiempo:** ~10 min
- **Qué hace:** stack mínimo, DB schema básico, BaaS decision (default Supabase, override Insforge solo si Tech Spec lo justifica).
- **Inputs requeridos:** `PDR-[nombre].md`.
- **Adaptación para MVP:**
  - Stack: Forja Golden Path (Next.js + Supabase) — no explorar alternativas.
...
### Step 5 · UI (Core Screens, R10 Brand DNA gate aplica)
...
- **⚠️ R10 Brand DNA gate:** aún en MVP, la UI consume `brand/brand.json` + `voice.json`. Para arrancar rápido, este step puede invocar `add-ui-kit` con preset Modern Minimal (default) si el usuario no tiene brand definida — fast path sin Discovery completa.
- **Adaptación para MVP:**
...
| Skill omitido | Razón |
|---------------|-------|
| UX Research (#4) | Personas y journey maps toman tiempo; en MVP hablás directamente con los usuarios |
...
| Security Audit (#9) | No hay datos de producción reales; se hace antes de escalar (handoff a `el-guardian` post-validación) |

---

## El MVP Valida — ¿Y Después?

Si el MVP valida la hipótesis, el siguiente paso es migrar a **`routes/saas-completo.md`**.
...
Wizards relevantes post-validación: `/init-saas` (bootstrap completo) y `/add-monetization` (pagos + emails + audit).
