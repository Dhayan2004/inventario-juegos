-- 0002_subscriptions.sql (Polar mode)
--
-- Identical shape al de Stripe — subscriptions + refund_requests con
-- RLS L-001. provider check incluye 'polar'. status enum cubre los 9
-- estados normalizados (Polar: active/canceled/revoked → mapean al enum).
--
-- Si add-payments corre Mode A (Stripe) y luego Mode B (Polar) en el mismo
-- proyecto, esta migration es idempotente — `create table if not exists`.
--
-- Cita:
--   [memory:lessons#L-001] (RLS by user_id)
--   [memory:CONSTRAINTS.md#R14] (refund_requests audit log)
